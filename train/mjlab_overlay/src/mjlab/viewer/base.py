...
class BaseViewer(ABC):
  ...
  def _dump_policy_obs(self, obs: Any) -> None:
    """Print the policy input for the first few steps."""
    ...
    if hasattr(tensor, "ndim") and tensor.ndim >= 2:
      tensor = tensor[0]
    elif hasattr(tensor, "shape") and len(tensor.shape) >= 2:  # type: ignore[arg-type]
      tensor = tensor[0]
    ...
    print(f"PYTHON_OBS [{name}] size={len(flat)}: {flat}")
