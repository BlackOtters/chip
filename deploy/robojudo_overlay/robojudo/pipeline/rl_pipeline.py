...
class RlPipeline(Pipeline):
    ...
    def _get_motion_start_blend_seconds(self) -> float:
        """Startup blend duration for MOTION_RESET/MOTION_FADE_IN.

        Only CHIP tracking config provides this field; fallback to 0 for
        other policies to preserve existing behavior.
        """
        cfg_policy = getattr(self.policy, "cfg_policy", None)
        if cfg_policy is None:
            return 0.0
        secs = getattr(cfg_policy, "motion_start_blend_seconds", 0.0)
        return max(float(secs), 0.0)
    ...
    def reset(self):
        ...
        self._pending_motion_start_blend = False
        ...
        for command in commands:
            match command:
                ...
                case "[MOTION_RESET]" | "[MOTION_FADE_IN]":
                    ...
                    if self._has_default_pose_mode:
                        blend_secs = self._get_motion_start_blend_seconds()
                        if blend_secs > 0.0:
                            # Safety transition: avoid abrupt jump from
                            # standing pose to frame-0 target (e.g. hands
                            # swinging behind back).
                            logger.info(
                                f"{command} — smooth motion start ({blend_secs:.2f}s blend)"
                            )
                            self._pending_motion_start_blend = True
                        else:
    ...
    def step(self, dry_run=False):
        ...
        if self._pending_motion_start_blend:
            self._pending_motion_start_blend = False
            self._run_motion_start_blend()
    ...
    def _run_motion_start_blend(self):
        """Blend current commanded pose -> policy output at motion frame 0.

        Used by default-pose tracker policies (CHIP) when user presses
        MOTION_RESET / MOTION_FADE_IN on real robot.
        """
        secs = self._get_motion_start_blend_seconds()
        blend_steps = max(int(secs * self.freq), 1)

        self.env.update()
        env_data = self.env.get_data()
        start_pd = np.asarray(env_data.dof_pos, dtype=np.float32).copy()

        logger.warning(
            f"Motion-start blend: current DOF -> policy frame0 ({blend_steps} steps, {secs:.2f}s)"
        )
        pbar = ProgressBar("Motion start blend", blend_steps)
        ...
        for t in range(blend_steps):
            ...
            action = (1 - alpha) * start_pd + alpha * policy_pd
            self.env.step(action, extras.get("hand_pose", None))
        ...
        # Keep policy in tracking mode after smooth start.
        self._set_default_pose_mode(False)
        logger.warning("Motion-start blend done — motion tracking running")
