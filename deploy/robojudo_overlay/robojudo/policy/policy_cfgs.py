...
class ChipTrackingPolicyCfg(PolicyCfg):
    """Config for CHIP tracking policy loaded from an RSL-RL checkpoint."""

    policy_type: str = "ChipTrackingPolicy"
    disable_autoload: bool = True

    checkpoint_file: str = ""
    motion_file: str = ""
    motion_time_start: float = 0.0
    motion_time_end: float | None = None
    start_in_default_pose: bool = True
    motion_start_blend_seconds: float = 2.0
    ...
    default_joint_pos_keyframe: dict[str, float] = {
    ...
    joint_pos_bias: list[float] | None = None

    compliance_range: list[float] = [0.0, 0.05]
    force_clip: float = 40.0
    min_force: float = 1.0
    max_goal_displacement: float = 0.5
    goal_smoothing_alpha: float = 0.2

    # Placeholder DoF configs, replaced at runtime by the policy implementation.
