...
class MujocoEnvCfg(EnvCfg):
    ...
    sim_ls_iterations: int | None = None

    # Optional per-joint dynamics overrides (key: joint name).
    # These write into MuJoCo dof arrays after model load.
    joint_armature_override: dict[str, float] | None = None
    joint_damping_override: dict[str, float] | None = None
    joint_frictionloss_override: dict[str, float] | None = None

    # Actuator compatibility mode:
    # - "torque_pd": RoboJuDo legacy mode (external torque PD in step()).
    # - "position_servo": set MuJoCo actuator params to position-servo form and
    #   feed desired joint position directly via data.ctrl.
    actuator_mode: Literal["torque_pd", "position_servo"] = "torque_pd"
    actuator_kp_override: dict[str, float] | None = None
    actuator_kd_override: dict[str, float] | None = None
    actuator_effort_override: dict[str, float] | None = None
